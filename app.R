...
school_level_choices <- intersect(
  c("Elementary", "Elementary+Middle","Middle", "Middle+High","High", "Unclear"),
  ...
  tags$head(
    tags$style(
      HTML('
        ...
        /* FIXED: All plotly elements should have pointer cursor */
        .plotly,
        .plotly *,
        .plotly .main-svg,
        .plotly svg,
        ...
        /* Specific override for school level plot */
        div[id*="school_level"],
        div[id*="school_level"] * {
          cursor: pointer !important;
        }
...
output$school_level <- renderPlotly({
  ...
  independent_data <- merged %>%
    ...
    filter(
      ...
      urbanicity_clean %in% input$urbanicity_filter,
      sapply(processed_grades, function(x) grades_match_filter(x, input$individual_grade_filter)),
      sapply(processed_school_types, function(x) school_types_match_filter(x, input$individual_school_type_filter)),
      sapply(processed_outcome_measure_roots, function(x) outcome_measures_match_filter_roots(x, input$outcome_family_filter))
    )
  
  # Apply cross-filters (unchanged)
  if (!is.null(cross_filters$countries)) {
    independent_data <- independent_data[independent_data$country %in% cross_filters$countries, ]
  }
  if (!is.null(cross_filters$urbanicity)) {
    independent_data <- independent_data[independent_data$urbanicity_clean %in% cross_filters$urbanicity, ]
  }
  if (!is.null(cross_filters$outcomes)) {
    independent_data <- independent_data[independent_data$processed_outcome_measure_roots %in% cross_filters$outcomes, ]
  }
  ...
  school_level_plot_data <- independent_data %>%
    ...
    mutate(
      grade_category = factor(
        grade_category,
        levels = rev(c("Elementary", "Elementary+Middle", "Middle", "Middle+High", "High", "Unclear"))
      )
    ) %>%
  ...
  # Use select dragmode for pointer cursor + proper hover styling
  ggplotly(p, source = "school_level_plot") %>%
    layout(
      margin = list(l = 5, r = 5, t = 5, b = 90),
      hoverlabel = list(bgcolor = "white", font = list(color = "black")),  # White background, black text
      xaxis = list(fixedrange = TRUE),
      yaxis = list(fixedrange = TRUE),
      dragmode = 'select'  # Pointer cursor everywhere
    ) %>%
    config(
    ...
    style(hovertemplate = "Studies: <b>%{x:.0f}</b><extra></extra>")  # Custom hover text
})
...
observeEvent(event_data("plotly_click", source = "school_level_plot"), {
  ...
  if (!is.null(click_data)) {
    y_pos <- round(click_data$y)
    available_categories <- rev(c("Elementary", "Elementary+Middle", "Middle", "Middle+High", "High", "Unclear"))
    
    if (y_pos >= 1 && y_pos <= length(available_categories)) {
      clicked_category <- available_categories[y_pos]
      current_selection <- input$school_level_filter
      
      new_selection <- if (clicked_category %in% current_selection) {
        current_selection[current_selection != clicked_category]
      } else {
        c(current_selection, clicked_category)
      }
