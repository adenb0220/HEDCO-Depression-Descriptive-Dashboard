...
'),
  ...
  fluidRow(
    ...
    column(2,
           ...
           # Move the CSS outside the button div
...
server <- function(input, output, session) {
  ...
  # Reactive values to store cross-filter selections
  cross_filters <- reactiveValues(
    countries = NULL,
    school_levels = NULL,
    urbanicity = NULL,
    outcomes = NULL
  ...
  # Function to reset all cross-filters
  reset_cross_filters <- function() {
    cross_filters$countries <- NULL
    cross_filters$school_levels <- NULL
    cross_filters$urbanicity <- NULL
    cross_filters$outcomes <- NULL
  }
  
  # Add reset button observer - UPDATED to clear ALL filters
  observeEvent(input$reset_filters, {
    # Clear cross-filters
    reset_cross_filters()
    
    # Reset all dropdown filters to their original "All" selections
  ...
  #Filters
  # Update the filtered_data() reactive to apply school level cross-filter to OTHER components
  filtered_data <- reactive({
    # If ANY filter is empty, return empty data frame
    if (
      is.null(input$country_filter) || length(input$country_filter) == 0 ||
    ...
    # Apply cross-filters
    if (!is.null(cross_filters$countries)) {
      data <- data %>% filter(country %in% cross_filters$countries)
    }
    
    # ADD THIS BACK - Apply school level cross-filter for OTHER dashboard components
    if (!is.null(cross_filters$school_levels)) {
      data <- data %>% filter(grade_category %in% cross_filters$school_levels)
      cat("Applied school level cross-filter:", paste(cross_filters$school_levels, collapse = ", "), "\n")
      cat("Rows after school level filter:", nrow(data), "\n")
    }
    
    if (!is.null(cross_filters$urbanicity)) {
      data <- data %>% filter(urbanicity_clean %in% cross_filters$urbanicity)
    }
    
    if (!is.null(cross_filters$outcomes)) {
      data <- data %>% filter(processed_outcome_measure_roots %in% cross_filters$outcomes)
    }
  ...
  output$school_level <- renderPlotly({
    # Create data source that shows ALL categories (not filtered by school_level_filter for THIS plot)
    independent_data <- merged %>%
      mutate(grade_category = sapply(grade_level, classify_grade_level)) %>%
      filter(country %in% input$country_filter) %>%
      # DON'T filter by school_level_filter here - we want to show all bars
      filter(urbanicity_clean %in% input$urbanicity_filter) %>%
      filter(sapply(processed_grades, function(x) grades_match_filter(x, input$individual_grade_filter))) %>%
      filter(sapply(processed_school_types, function(x) school_types_match_filter(x, input$individual_school_type_filter))) %>%
      filter(sapply(processed_outcome_measure_roots, function(x) outcome_measures_match_filter_roots(x, input$outcome_family_filter)))
    
    # Apply OTHER cross-filters (but NOT school level)
    if (!is.null(cross_filters$countries)) {
      independent_data <- independent_data %>% filter(country %in% cross_filters$countries)
    }
    
    if (!is.null(cross_filters$urbanicity)) {
      independent_data <- independent_data %>% filter(urbanicity_clean %in% cross_filters$urbanicity)
    }
    
    if (!is.null(cross_filters$outcomes)) {
      independent_data <- independent_data %>% filter(processed_outcome_measure_roots %in% cross_filters$outcomes)
    }
    
    school_level_plot_data <- independent_data %>%
      distinct(study, grade_category) %>%
      mutate(
        grade_category = factor(
          grade_category,
          levels = c("Elementary", "Elementary+Middle", "Middle", "Middle+High", "High", "Unclear")
        )
      ) %>%
      group_by(grade_category) %>%
      summarise(grade_n = n(), .groups = "drop") %>%
      mutate(
        # Selection is based on the DROPDOWN filter (for visual transparency)
        selected = as.character(grade_category) %in% input$school_level_filter,
        # Create colors: FULL OPACITY for selected, 50% opacity for deselected
        fill_color = case_when(
          selected & grade_n <= 5 ~ "#8ABB40",     # Low values, selected
          selected & grade_n <= 10 ~ "#489D46",    # Medium-low values, selected  
          selected & grade_n <= 15 ~ "#007030",    # Medium-high values, selected
          selected ~ "#104735",                    # High values, selected
          !selected & grade_n <= 5 ~ "#8ABB4080",  # Low values, deselected
          !selected & grade_n <= 10 ~ "#489D4680", # Medium-low values, deselected
          !selected & grade_n <= 15 ~ "#00703080", # Medium-high values, deselected  
          TRUE ~ "#10473580"                       # High values, deselected
        )
      )
    
    cat("=== PLOT RENDER DEBUG ===\n")
    cat("dropdown filter:", paste(input$school_level_filter, collapse = ", "), "\n")
    cat("Categories and selection:", paste(school_level_plot_data$grade_category, "=", school_level_plot_data$selected, collapse = "; "), "\n")
    cat("Data rows:", nrow(school_level_plot_data), "\n")
    cat("=== END PLOT DEBUG ===\n")
    ...
    # Convert to plotly with fixed hover template
    ggplotly(p, source = "school_level_plot") %>%
      layout(
      ...
      # Fix hover template to show proper text
      style(
        hovertemplate = paste0(
          "<span style='font-size:14px;'>School Level: <b>%{x}</b><br>",
          "Number of Studies: <b>%{y}</b></span><extra></extra>"
        )
      ) %>%
  ...
  # Handle school level plot clicks - Update dropdown to match clicks
  observeEvent(event_data("plotly_click", source = "school_level_plot"), {
    cat("=== CLICK EVENT ===\n")
    
    click_data <- event_data("plotly_click", source = "school_level_plot")
    
    if (!is.null(click_data)) {
      x_pos <- round(click_data$x)
      available_categories <- c("Elementary", "Elementary+Middle", "Middle", "Middle+High", "High", "Unclear")
      
      if (x_pos >= 1 && x_pos <= length(available_categories)) {
        clicked_category <- available_categories[x_pos]
        cat("Clicked:", clicked_category, "\n")
        
        # Get current dropdown selection
        current_selection <- input$school_level_filter
        
        # Toggle the clicked category in the dropdown
        if (clicked_category %in% current_selection) {
          # Deselect
          new_selection <- current_selection[current_selection != clicked_category]
        } else {
          # Reselect
          new_selection <- c(current_selection, clicked_category)
        }
        
        # Update dropdown
        updatePickerInput(session, "school_level_filter", selected = new_selection)
        
        cat("Updated dropdown from:", paste(current_selection, collapse = ", "), "\n")
        cat("Updated dropdown to:", paste(new_selection, collapse = ", "), "\n")
      }
    }
